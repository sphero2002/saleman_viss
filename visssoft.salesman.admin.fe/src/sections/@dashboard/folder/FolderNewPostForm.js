                    ...
                    <div key={item.code}>
                      ...
                      />
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                            <span style={{ fontSize: '0.875rem', fontWeight: 400, width: '200px' }}>Đóng/Mở tài liệu</span>
                                            <Select
                                                name={`items[${index}].periodid`}
                                                fullWidth
                                                value={periodid}
                                                select
                                                onChange={(event) => handleChange(event, index)}
                                                helperText="Chọn thời gian để đóng mở tài liệu"
                                            >
                                                {periods.map((option) => (
                                                    <MenuItem key={option.id} value={option.id} >
                                                        {option.description} : {moment(option.fromdate).format("DD/MM/YYYY")} {'->'} {moment(option.todate).format("DD/MM/YYYY")}
                                                    </MenuItem>
                                                ))}
                                            </Select>
                                        </div>
